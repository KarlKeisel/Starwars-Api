// Starwars API is small, maybe only 300 total objects. (More so with general pages also displaying data)
// Use a variable to store all data for each page visit, so backtracking will check here first then API call.

// Sample page has info loaded into "results", each page has list of 10 results each with a link to themselves in their own results.
// TODO have a system to read the JSON data and apply it to a apiData, with "url" as object name and info as data.
// TODO Before each API call, check if "url" is in apiData, if so pull from that instead.
// Normally if it was bigger data or worried about data changing, I would use a database instead.

let apiData = {};  // Will save all data that comes through the API
